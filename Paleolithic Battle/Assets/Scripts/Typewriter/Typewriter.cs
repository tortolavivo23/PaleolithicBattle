    ...
    private void Update()
    {
        ...
        if (currentMessage != null && currentMessage.IsActive())
        {
            if (!songPlaying)
            {
                if (speaker)
                {
                    AudioManager.Instance.Play("TypewriterA");
                }
                else
                {
                    AudioManager.Instance.Play("TypewriterB");
                }
                songPlaying = true;
            }
        }
            ...
            if (songPlaying)
            {
                if(speaker)
                {
                    AudioManager.Instance.Stop("TypewriterA");
                }
                else
                {
                    AudioManager.Instance.Stop("TypewriterB");
                }
                speaker = !speaker;
                songPlaying = false;
            }
